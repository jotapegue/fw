...
class usuario extends sql{
	...
	function __construct( $protocolo = NULL ){
	   ...
	   parent::__construct('usu');
		...
		( !empty( $_POST['usu_tiragem'] ) )				? $this->tiragem				= $_POST['usu_tiragem']							 : false;
	...
	private function setFoto(){
		...
		if( !$_FILES['3x4']['error'] == 4 ){
			$this->foto = $this->formulario . ".jpg";
				$upload = $this->foto;
					move_uploaded_file( $_FILES['3x4']['tmp_name'], $upload );
		}else{
			return $this->foto = "sem-foto.jpg";
		}
	...
	function editUser(){
		...
		if( !$_FILES['3x4']['error'] == 4 ){
			$set =
				...
				self::data('abreviatura_carteira') . "='{$this->abreviatura_carteira}',".
				...
				self::data('tiragem') . "='{$this->tiragem}',".			
